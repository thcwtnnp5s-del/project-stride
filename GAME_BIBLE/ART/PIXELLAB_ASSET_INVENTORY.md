...
| Fauna stills ×4, 16² (Iteration 02) | `RCP01/out/fauna/fauna_{hare,songbird,crow,ptarmigan}_16` (accepted; sources now tracked) | `ambient/fauna_*_16_f0.png` — one region creature composed into every solo ambient scene via `AmbientAssets.scenesFor` (hare/Haven, songbird/Woods, crow/Hollow, ptarmigan/Frostmere); Stonefall empty — `fauna_bat_16` stays withheld |
| Traveler walk west, 6f 64² (Iteration 02) | `PLAYABLE_EXPANSION_01/combat/candidates/trav_zip/…/traveler_walk/west/` (the production Traveler's own set; east not shipped — vest vanishes f0–f1; sources now tracked) | `anim/traveler_walk_west_f0..5.png` — the travel transition card on committed arrivals (~1.3 s over the destination's alt vignette; Reduce Motion skips) |
...
|---|---|---|---|
| Fauna pack remainder — butterfly stills/loop (24 px), the 32-px stills, the songbird/crow/hare/bat loops | `RCP01/out/fauna/` (sources tracked since Iteration 02) | The four 16-px stills shipped (Iteration 02, §2). The loops are 24/32-px canvases the 16-px stage slots don't take, and an animated companion per location is a step past "quiet life" the stills should prove out first | If the stills land on device, a loops round is the natural sequel; the 32-px set waits for a larger surface |
...
| Traveler walk cycles beyond west — the trav_zip east/north/south + diagonals, and the older PROOF_02 8-direction set | `PLAYABLE_EXPANSION_01/…/traveler_walk/`, `PIXELLAB_PROOF_02/out/animation/` | West shipped (Iteration 02, §2 — the travel card needs one clean direction). The trav_zip east loses its vest on f0–f1; the PROOF_02 set carries a backpack flicker and predates the production character; mirroring frames is a creative change PixelLab owns (A-2) | A direction-QA round if a use for more directions appears (route-line animation); otherwise west suffices |

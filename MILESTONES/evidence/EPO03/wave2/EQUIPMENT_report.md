...
**Four of DIR-08's five top failures are closed in code and in art.** The
...
test/craft_stage_evidence_test.dart` — **25 passing.** `flutter analyze` clean on
every file touched. `node Scripts/art/package-art.js --check` — **2,203 files up
to date**.
...
2. **The Fanghilt Sword (`weapon.fang`, P5, ≈245).** `item.fanghilt_sword`
...
3. **The warden carries no backpack.** Every other body does. The cloak covers
...
4. **The longsword's blade midtone runs lighter than the bronze sword's.** The
...
5. **The warden's contact footprints are 40–41 px wide** on the longsword
...
6. **The base body's `weapon.steel` gather column and the base brace's own
...
| `tool.{axe,pick}.special` | goblin_toothed, hornbound, hornpoint | see below | see below |

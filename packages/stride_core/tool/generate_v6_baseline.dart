// Generates `test/fixtures/save/v6_baseline.save`, once.
//
// ## Run this exactly once, and never again
//
// ```text
// dart run tool/generate_v6_baseline.dart
// ```
//
// The output is a **frozen fixture** on the same terms as `v1_baseline.save`
// through `v5_baseline.save`: it represents a save written by a build that
...
// file, **that is the test doing its job.** The fix is a state version 7, a new
// decoder, a new `StateMigrations` step, and a new fixture — never a re-run of
// this script.
//
// ## What it produces
//
// The v5 baseline fixture, put through the real v5→v6 step of the migration
// table (`DECISIONS/0022`, finite background activity):
//
// ```text
// decode v5_baseline.save              → granted 1041, spent 400,
//                                        epoch (1041, 400) @v3, banked 0,
//                                        encounter null, visitVictories {}
// migratedToCurrentVersion()           → same figures, stateVersion 6,
//                                        activityQueue null
// (no re-basing step on the path)      → zero events
// encodeSnapshot(same id/gen/lastTx)   → v6_baseline.save
// ```
//
// The v5→v6 step declares `rebasesEconomy: false`, so the engine issues no
// command and the epoch does not move. What the fixture proves is exactly and
// only the *format* change: `activityQueue` (null, here — a v5 save could not
// carry a queue, so its migration cannot invent one) and the version.
//
...
void main() {
  ...
  final File source = File('${fixtures.path}/v5_baseline.save');
  final File target = File('${fixtures.path}/v6_baseline.save');
  ...
  if (!framed.verified) {
    stderr.writeln('The v5 fixture does not verify: ${framed.fault}');
    exitCode = 1;
    return;
  }
  final SaveEnvelope envelope = decodeEnvelope(framed.payload!);

  if (envelope.state.stateVersion != 5) {
    stderr.writeln(
      'The v5 fixture reports state version ${envelope.state.stateVersion}. '
      'This script generates the v6 fixture from a v5 save and nothing else.',
    );
    exitCode = 1;
    return;
  }
  if (StateVersion.current.value != 6) {
    stderr.writeln(
      'This build is at ${StateVersion.current}. This script produces a v6 '
  ...
  final StateMigrationApplication applied = applyStateMigrationPath(
    ...
    path: StateMigrations.pathFrom(5),
  );
  if (applied is! StateMigrationApplied) {
    stderr.writeln('The v5→v6 path was refused: $applied');
    exitCode = 1;
    return;
  }
  if (applied.events.isNotEmpty) {
    stderr.writeln(
      'The v5→v6 path produced ${applied.events.length} event(s); a reshape-'

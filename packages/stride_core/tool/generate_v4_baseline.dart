// Generates `test/fixtures/save/v4_baseline.save`, once.
//
// ## Run this exactly once, and never again
//
// ```text
// dart run tool/generate_v4_baseline.dart
// ```
//
// The output is a **frozen fixture** on the same terms as `v1_baseline.save`,
// `v2_baseline.save` and `v3_baseline.save`: it represents a save written by a
// build that will eventually no longer exist, which is the only thing a
// player's phone actually contains. Regenerating it against a later build
// deletes that evidence and replaces it with a restatement of whatever the code
// does that day — after which the round-trip test in `save_migration_test.dart`
// is a tautology.
//
// If a future change makes this generator's output differ from the checked-in
// file, **that is the test doing its job.** The fix is a state version 5, a new
// decoder, a new `StateMigrations` step, and a new fixture — never a re-run of
// this script.
//
// ## What it produces
//
// The v3 baseline fixture, put through the real v3→v4 step of the migration
// table (`DECISIONS/0020`, Combat Slice 01):
//
// ```text
// decode v3_baseline.save              → granted 1041, spent 400,
//                                        epoch (1041, 400) @v3, banked 0
// migratedToCurrentVersion()           → same figures, stateVersion 4,
//                                        encounter null, drivenOff empty
// (no re-basing step on the path)      → zero events
// encodeSnapshot(same id/gen/lastTx)   → v4_baseline.save
// ```
//
// The v3→v4 step declares `rebasesEconomy: false`, so the engine issues no
// command and the epoch does not move. What the fixture proves is exactly and
// only the *format* change: two new keys (`encounter: null`,
// `world.drivenOff: []`) and the version. A fixture carrying a fight in
// progress is deliberately not the baseline — the round trip of an active
// encounter is proved in `combat_test.dart` against a state the engine
// produced, and the baseline stays the migrated lineage of the older fixtures.
//
...
void main() {
  ...
  final File source = File('${fixtures.path}/v3_baseline.save');
  final File target = File('${fixtures.path}/v4_baseline.save');
  ...
  if (!framed.verified) {
    stderr.writeln('The v3 fixture does not verify: ${framed.fault}');
    exitCode = 1;
    return;
  }
  final SaveEnvelope envelope = decodeEnvelope(framed.payload!);

  if (envelope.state.stateVersion != 3) {
    stderr.writeln(
      'The v3 fixture reports state version ${envelope.state.stateVersion}. '
      'This script generates the v4 fixture from a v3 save and nothing else.',
    );
    exitCode = 1;
    return;
  }
  if (StateVersion.current.value != 4) {
    stderr.writeln(
      'This build is at ${StateVersion.current}. This script produces a v4 '
  ...
  final StateMigrationApplication applied = applyStateMigrationPath(
    ...
    path: StateMigrations.pathFrom(3),
  );
  if (applied is! StateMigrationApplied) {
    stderr.writeln('The v3→v4 path was refused: $applied');
    exitCode = 1;
    return;
  }
  if (applied.events.isNotEmpty) {
    stderr.writeln(
      'The v3→v4 path produced ${applied.events.length} event(s); a reshape-'

...
| ~~`world/atlas_base.png`, `world/atlas_south.png`, `world/landmark_*.png` (5)~~ | — | — | **Retired by Activity Feel 01**: the tile column and every landmark cutout are replaced by the one master painting below (their sources stay tracked in the exploration rounds) |
...
| **World & Reward Depth 01** — `ambient/traveler_read_*` (9 f, replaces the PE01 read at book scale), `ambient/traveler_idle_breathe_*` (7 f), `ambient/traveler_look_around_*` (7 f) | 64 × 64 | ×2 | `WORLD_REWARD_DEPTH_01/ambient/out/ambient/` via `manifest.json` (blind QA PASS / PASS-WITH-NOTE); supersede the same ids in the earlier passes. `traveler_pick_inspect` frames stay packaged but are out of the rotation (blind QA FAIL) |
| `combat/lynx_{idle,attack,defeat}_*.png`, `combat/backdrop_frostmere.png` | 56 × 56 (anchor row 39), 192 × 96 | ×2 | `WORLD_REWARD_DEPTH_01/combat/out/combat/` via `manifest.json`, `status: accepted` only (`lynx_hit` withheld, not emitted) |
| `item/{wolf_pelt,lynx_pelt,wolfhide_jerkin,frostlined_jerkin}.png` | 48 × 48 | 48 | `WORLD_REWARD_DEPTH_01/items/out/` (blind QA PASS; lynx pelt PASS-WITH-NOTE) |
| `world/marker_{haven,wilds,worksite,perilous,landmark}.png` | 20 × 20 | ×4 on the atlas, counter-scaled | `WORLD_REWARD_DEPTH_01/world/out/world/` (the round's south tile is retired with the base — see the Activity Feel 01 rows) |
| **Activity Feel 01** — `world/atlas_master.png` | 384 × 688 | 1536 × 2752 (×4, pannable; the whole world frames at the 0.25 floor) | `ACTIVITY_FEEL_01/out/world/atlas_master_384x688.png` — ONE `create_image_pro` painting, blind QA PASS-WITH-NOTE ("CLEAR IMPROVEMENT" over the tile world); r2 carries one 96×96 cave inpaint. A single generation cannot have a seam — `MISTAKES.md` M-12 applied |
| `ambient/activity_{woodcut(8f),mine(6f),forage(9f)}_f*.png` | 76/66/44 × 64, feet on row 62 | ×2 on the gather stage, feet-centre aligned | `ACTIVITY_FEEL_01/out/ambient/` — the working loops a gathering queue plays; frame selection and crop boxes in `package-art.js`; foraging plays ping-pong from the Dart table |
| `env/overlay_birds_f0..5.png` | 24 × 24 | ×4, drifting | `ACTIVITY_FEEL_01/out/env/` — seamless loop (last frame pinned to the first) |
